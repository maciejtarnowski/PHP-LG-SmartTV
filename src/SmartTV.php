...
class SmartTV
{
    ...
    {
        $this->connectionDetails = [$ipAddress, $port];
    }

    /**
     * @param int $pairingKey
     */
    ...
    public function displayPairingKey()
    {
        $this->sendXMLRequest(
            '/roap/api/auth',
            self::encodeData(['type' => 'AuthKeyReq'], 'auth')
        );
    }
    ...
    {
        $url = $this->connectionDetails[0] . ':' . $this->connectionDetails[1] . $actionFile;
